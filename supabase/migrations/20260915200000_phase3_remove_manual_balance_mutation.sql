-- Phase 3: legacy inventory totals are transitional caches, never writable by browser clients.
-- Canonical balances are public.mill_cash_balance and public.mill_oil_balance.
BEGIN;

DROP POLICY IF EXISTS "Tenant isolation for inventory" ON public.inventory;

CREATE POLICY "inventory read by tenant members"
ON public.inventory
FOR SELECT
TO authenticated
USING (
...
REVOKE INSERT, UPDATE, DELETE ON TABLE public.inventory FROM PUBLIC, anon, authenticated;
...
BEGIN
  ...
  IF NOT public.has_active_mill_role(v_mill, ARRAY['mill_owner']) THEN
    RAISE EXCEPTION USING ERRCODE = 'P0001', MESSAGE = 'OPENING_BALANCE_FORBIDDEN';
  END IF;

  IF p_amount IS NULL OR p_amount <= 0 THEN
    RAISE EXCEPTION USING ERRCODE = 'P0001', MESSAGE = 'OPENING_BALANCE_INVALID';
  END IF;
  ...
  VALUES (
    v_mill, p_season_id, 'cash_opening_balance', 'cash_opening_balance', v_actor
  )
  RETURNING id INTO v_operation;

  INSERT INTO public.financial_transactions(
    created_by, mill_id, season_id, type, category, amount, direction,
    payment_method, reference_type, description, status, operation_id, idempotency_key
  )
  VALUES (
    v_actor, v_mill, p_season_id, 'adjustment', 'opening_balance', p_amount, 'in',
    'cash', 'cash_opening_balance',
    COALESCE(NULLIF(BTRIM(p_notes), ''), 'الرصيد النقدي الافتتاحي'),
  ...
  PERFORM private.complete_business_command(
    p_idempotency_key,
    'cash_opening_balance',
    jsonb_build_object('success', true, 'financial_transaction_id', v_financial),
    v_operation
  );
...
BEGIN
  ...
  VALUES (
    v_mill, p_season_id, 'oil_opening_balance', 'oil_opening_balance', v_actor
  )
  RETURNING id INTO v_operation;

  INSERT INTO public.oil_movements(
    mill_id, season_id, ownership, direction, movement_type, source_type,
    amount, quantity, unit_price, notes, reference_type, reference_id,
    created_by, idempotency_key
  )
  VALUES (
    v_mill, p_season_id, 'mill', 'in', 'IN', 'opening_balance',
    p_amount, p_amount, 0,
    COALESCE(NULLIF(BTRIM(p_notes), ''), 'الرصيد الافتتاحي للزيت'),
    'oil_opening_balance', v_operation,
  ...
  PERFORM private.complete_business_command(
    p_idempotency_key,
    'oil_opening_balance',
    jsonb_build_object('success', true, 'oil_movement_id', v_movement),
    v_operation
  );
