...
const HeaderBar = () => {
  ...
  return (
    <header className="min-h-16 border-b glass-bar flex items-center justify-between gap-2 px-3 py-2 sm:px-4 md:px-6 sticky top-0 z-40">
      <div className="flex min-w-0 items-center gap-2 sm:gap-3">
        ...
        {!isAdmin && activeSeason && (
          <Badge
            variant="secondary"
            className={`max-w-[calc(100vw-8rem)] text-xs font-medium px-2.5 py-1.5 rounded-full border border-primary/20 transition-colors flex items-center gap-1.5 ${canNavigateToSeasons ? "cursor-pointer hover:bg-primary/10" : "cursor-default"
              }`}
            onClick={() => {
          ...
          >
            ...
            <span className="truncate text-primary font-semibold">{activeSeason.name}</span>
          </Badge>
        )}
        ...
        {isAdmin && (
          <Badge
            variant="secondary"
            className="hidden sm:inline-flex bg-amber-500/10 text-amber-700 dark:text-amber-400 border border-amber-500/20 text-xs font-semibold px-3 py-1.5 rounded-full"
          >
