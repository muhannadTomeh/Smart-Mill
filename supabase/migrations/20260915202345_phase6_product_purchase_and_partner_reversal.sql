...
ALTER TABLE public.product_purchases ADD CONSTRAINT product_purchases_status_check CHECK (status IN ('active','cancelled'));
ALTER TABLE public.product_purchases DROP CONSTRAINT IF EXISTS product_purchases_cancellation_consistent;
ALTER TABLE public.product_purchases ADD CONSTRAINT product_purchases_cancellation_consistent CHECK (
  (status='active' AND cancelled_at IS NULL AND cancelled_by IS NULL AND cancellation_reason IS NULL AND cancellation_operation_id IS NULL)
  OR (status='cancelled' AND cancelled_at IS NOT NULL AND cancelled_by IS NOT NULL AND nullif(btrim(cancellation_reason),'') IS NOT NULL AND cancellation_operation_id IS NOT NULL)
);

CREATE OR REPLACE FUNCTION public.cancel_product_purchase_command(
  p_purchase_id uuid, p_reason text, p_idempotency_key uuid
) RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path = public, extensions AS $$
DECLARE
  a uuid:=auth.uid(); p public.product_purchases%ROWTYPE; payable public.payables%ROWTYPE;
  fin public.financial_transactions%ROWTYPE; previous jsonb; op uuid; original_op uuid;
  reverse_fin uuid; reverse_stock uuid; stock integer;
BEGIN
  ...
  SELECT * INTO p FROM public.product_purchases WHERE id=p_purchase_id FOR UPDATE;
  ...
  SELECT current_stock INTO stock FROM public.products WHERE id=p.product_id AND mill_id=p.mill_id FOR UPDATE;
  IF NOT FOUND THEN RAISE EXCEPTION USING ERRCODE='P0001',MESSAGE='PRODUCT_NOT_FOUND'; END IF;
  IF stock < p.quantity THEN RAISE EXCEPTION USING ERRCODE='P0001',MESSAGE='INSUFFICIENT_STOCK_FOR_CANCELLATION'; END IF;
  SELECT * INTO payable FROM public.payables WHERE source_type IN ('product_purchase','purchase') AND source_id=p.id FOR UPDATE;
  IF FOUND AND payable.paid_amount > 0 THEN RAISE EXCEPTION USING ERRCODE='P0001',MESSAGE='DEPENDENT_SETTLEMENT_EXISTS'; END IF;
  SELECT * INTO fin FROM public.financial_transactions WHERE reference_type='product_purchase' AND reference_id=p.id AND reversal_of IS NULL ORDER BY created_at LIMIT 1 FOR UPDATE;
  IF NOT FOUND THEN RAISE EXCEPTION USING ERRCODE='P0001',MESSAGE='PRODUCT_PURCHASE_LEDGER_EVENT_NOT_FOUND'; END IF;
  IF EXISTS(SELECT 1 FROM public.financial_transactions WHERE reversal_of=fin.id) THEN RAISE EXCEPTION USING ERRCODE='P0001',MESSAGE='PRODUCT_PURCHASE_ALREADY_CANCELLED'; END IF;
  ...
  VALUES(p.mill_id,p.season_id,'product_purchase_cancellation','product_purchase',p.id,a,original_op) RETURNING id INTO op;
  ...
  VALUES(p.mill_id,p.season_id,p.product_id,-p.quantity,'adjustment','product_purchase_cancellation',p.id,'إلغاء شراء بضاعة: '||btrim(p_reason),a,p_idempotency_key) RETURNING id INTO reverse_stock;
  ...
  VALUES(a,p.mill_id,p.season_id,'adjustment','product_purchase_cancellation',fin.amount,
    (CASE fin.direction WHEN 'in'::public.financial_direction THEN 'out' WHEN 'out'::public.financial_direction THEN 'in' ELSE 'none' END)::public.financial_direction,
    fin.payment_method,'product_purchase_cancellation',p.id,fin.party_type,fin.party_id,fin.party_name,'إلغاء شراء بضاعة: '||btrim(p_reason),'active',op,p_idempotency_key,fin.id,btrim(p_reason)) RETURNING id INTO reverse_fin;
...
CREATE OR REPLACE FUNCTION public.reverse_partner_transaction_command(
  p_financial_transaction_id uuid, p_reason text, p_idempotency_key uuid
) RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path = public, extensions AS $$
DECLARE a uuid:=auth.uid(); f public.financial_transactions%ROWTYPE; previous jsonb; op uuid; reverse_fin uuid;
BEGIN
  IF a IS NULL THEN RAISE EXCEPTION USING ERRCODE='P0001',MESSAGE='AUTHENTICATION_REQUIRED'; END IF;
  IF nullif(btrim(p_reason),'') IS NULL THEN RAISE EXCEPTION USING ERRCODE='P0001',MESSAGE='CANCELLATION_REASON_REQUIRED'; END IF;
  SELECT * INTO f FROM public.financial_transactions WHERE id=p_financial_transaction_id FOR UPDATE;
  IF NOT FOUND OR f.reference_type<>'partner_transaction' OR f.reversal_of IS NOT NULL THEN RAISE EXCEPTION USING ERRCODE='P0001',MESSAGE='PARTNER_TRANSACTION_NOT_REVERSIBLE'; END IF;
  IF NOT public.is_platform_admin(a) AND NOT public.has_active_mill_role(f.mill_id,ARRAY['mill_owner']) THEN RAISE EXCEPTION USING ERRCODE='P0001',MESSAGE='PARTNER_TRANSACTION_REVERSE_FORBIDDEN'; END IF;
  IF EXISTS(SELECT 1 FROM public.financial_transactions WHERE reversal_of=f.id) THEN RAISE EXCEPTION USING ERRCODE='P0001',MESSAGE='PARTNER_TRANSACTION_ALREADY_REVERSED'; END IF;
  ...
  INSERT INTO public.business_operations(mill_id,season_id,operation_type,source_type,source_id,created_by,reverses_operation_id)
  VALUES(f.mill_id,f.season_id,'partner_transaction_reversal','financial_transaction',f.id,a,f.operation_id) RETURNING id INTO op;
  INSERT INTO public.financial_transactions(created_by,mill_id,season_id,type,category,amount,direction,payment_method,reference_type,reference_id,party_type,party_id,party_name,description,status,operation_id,idempotency_key,reversal_of,reversal_reason)
  VALUES(a,f.mill_id,f.season_id,'adjustment','partner_transaction_reversal',f.amount,
    (CASE f.direction WHEN 'in'::public.financial_direction THEN 'out' ELSE 'in' END)::public.financial_direction,
  ...
  PERFORM private.complete_business_command(p_idempotency_key,'reverse_partner_transaction',jsonb_build_object('success',true,'financial_reversal_id',reverse_fin),op);
  RETURN jsonb_build_object('success',true,'financial_reversal_id',reverse_fin);
END; $$;
