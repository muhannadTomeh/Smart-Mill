...
export default function Seasons() {
  ...
  useEffect(() => {
    if (seasons.length > 0) fetchAllStats();
  }, [seasons]);
  ...
  const fetchAllStats = async () => {
    ...
    for (const season of seasons) {
      const [custRes, invRes] = await Promise.all([
        supabase.from("customers").select("id", { count: "exact", head: true }).eq("season_id", season.id),
        supabase.from("invoices").select("oil_produced, cash_amount").eq("season_id", season.id),
      ]);
