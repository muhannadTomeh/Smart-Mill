...
`credential-vault` Edge Function AES-GCM encrypts account passwords/admin PINs with a server secret and audits reveal. This is technically encrypted but retaining recoverable account passwords is a high-risk anti-pattern: passwords should be reset, never revealed. The endpoint does allow self-store and should be reviewed against Auth password changes. The current client fallback paths can directly update identity-related rows; remove them after server commands are reliable.
